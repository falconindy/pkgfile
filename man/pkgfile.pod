...
and zsh implementations.

=back

=head1 AUTOMATING UPDATES

It is recommended to use B<pkgfiled>(1) to keep .pfdb DBs up as `pacman -Fy` is
...
periodic updates to the .pfdb DBs. Assuming that pkgfile was built with
