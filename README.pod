...
may be one of B<none>, B<gzip>, B<bzip2>, B<lzop>, B<lz4>, B<lzma>, or B<xz>. If
this flag is passed without a compression method, this defaults to B<gzip>. If this
flag is not passed at all, no compression will be applied. Applying any form of
compression will decrease performance, but may be desirable for disk space
concerns.
...
=head1 PERIODIC UPDATES

pkgfile includes systemd timer and service units to automate periodic updates
to the .files DBs. Assuming that pkgfile was built with systemd support, this
can be enabled with:
